...
@dag(
    ...
    default_view='grid',
    orientation='TB',
    tags=["scRNA", "analysis", "10XGenomics", "hashing"]
)
def dag52_scRNA_10X_hashing():
    ## TASK
    ...
    work_dir = create_main_work_dir(
        task_tag='scRNA_hashing_output'
    )
    ## TASK - Configure hashing / Flex Pipeline
    analysis_script_info = prepare_cellranger_flex_script(
        design_dict=design,
        work_dir=work_dir
    )
    ## TASK - Execute hashing / Flex Pipeline
    cellranger_output_dir = run_cellranger_script(
        script_dict=analysis_script_info
    )
    ## TASK - fetch hashing samples
    sample_list = flex_sample_factory(
